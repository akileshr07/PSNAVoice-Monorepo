...
const CommentSection: React.FC<CommentSectionProps> = ({ complaintId }) => {
  ...
  const fetchComments = async () => {
    try {
      const res = await fetch(`http://localhost:8083/api/complaints/${complaintId}/comments`);
  ...
  const handleVote = async (commentId: string, isUpvote: boolean) => {
    try {
      const res = await fetch(
        `http://localhost:8083/api/complaints/${complaintId}/comments/${commentId}/vote?type=${isUpvote ? 'up' : 'down'}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        }
